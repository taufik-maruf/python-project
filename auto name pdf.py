...
def process_pdfs(pdf_dir, output_excel, start_keyword, end_keyword):
    data = [['File Name', 'Text']]
    for file_name in os.listdir(pdf_dir):
        if file_name.endswith('.pdf'):
            ...
            new_name = text.replace('\\', '').replace('/','').replace('/','').replace('-','').replace(':','').replace('(','').replace(')','').replace('\n','').replace('Place of Birth','').replace('Nomor Induk Mahasiswa','').replace('Student Number','') # Menghapus karakter "/"
            rename_and_move_file(file_path, new_dir, new_name)

    write_to_excel(output_excel, data)

# Contoh penggunaan:
process_pdfs('C:/0 Files/CSDU/Python/SKPI/Before', 'C:/0 Files/CSDU/Python/SKPI/Excel.xlsx', 'Name', 'Tempat Lahir') #:\n00
